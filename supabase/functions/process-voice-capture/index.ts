...
serve(async (req) => {
  ...
  try {
    const { transcription, audioUrl, userId } = await req.json();

    if (!transcription || !userId) {
      return new Response(
        JSON.stringify({ error: 'Transcription and userId are required' }),
    ...
    console.log('Processing voice capture for user:', userId);
...
Responde SOLO con un JSON válido con esta estructura exacta:
{
  ...
  "tags": ["etiquetas relevantes"]
    ...
    console.log('Voice idea saved successfully:', idea.id);
